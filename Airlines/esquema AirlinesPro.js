...
function priceAverage() {
  ...
  alert(
    `The average cost of today's ${flights.length} flights is ${average.toFixed(
      ...
      flights.length
    } flights that have a scale is ${scaleCount}.`
  );
}
...
function last5() {
  ...
  alert(`Thank you for using JS airlines, ${user}.`); // encadenar aquí con PRO
}
...
function askCredentials() {
  ...
  if (credentials === "admin") {
    console.log(`You have ${credentials} credentials.ADMIN`);
  }
  if (credentials === "user") {
    console.log(`You have ${credentials} credentials.USER`);
  } else {
    askCredentials();
  }
}
...
function flightCreationProcess(id, to, from, cost, scale) {
  indexer();
  flightFactory(id, to, from, cost, scale);
}

flightCreationProcess(15, "Panamá", "Jordan", 1111, "no");

let answer;
function question() {
  answer = prompt(
    `Do you want to CREATE or DELETE a flight?\nType CREATE or DELETE and hit accept.`
  ).toLowerCase();
}
let nuevoVuelo;
if (answer === "create") {
  nuevoVuelo.id = prompt("Input ID of the flight");
}
//question()
